...
- **FR-003:** When a coordinate is banned, the system MUST record a reason identifying the ban as automated and naming the forked repository, and MUST mark the candidate row `RESOLVED` with the same text in `notes`.
...
- **FR-006:** A candidate whose `(project_id, artifact_id)` no longer spans more than one `groupId` MUST NOT be banned. This is judged against the conflict set as it stands at the start of a run, so banning one member MUST NOT disqualify its siblings within that same run.
...
- **Modules touched:** `core/package` gains the qualifying query; `integrations/github` gains fork information on its repository model; `app` gains one scheduled job. `core/project` is reused unchanged. `core/package` depends on neither `core/project` nor `integrations/github`, so the ban call and the fork lookup are orchestrated from `app`, not from the module holding the query.
- **Database:** no schema change — no tables, columns, or migrations. Reads `suspicious_package_candidate` (KTL-4790), `package`, `project`, `scm_repo`, `scm_owner`, `package_dependency`; writes `banned_packages` and candidate `status`/`notes`. Implementation therefore depends on KTL-4790 being merged.
...
- **External integrations:** GitHub, one lookup per distinct `(suspect owner, repository)`. `GitHubIntegration.getRepository(owner, name)` exists but `GitHubRepository` carries no fork information, so the model and its Kohsuke-backed implementation must expose `fork` and the parent's full name.
- **Scheduled jobs:** one new daily `@Scheduled` in `app`, gated on `klibs.indexing`, with its own ShedLock name and an explicit `lockAtMostFor`.
...
- **Why:** one call per pair, reusing `getRepository(owner, name)` with two new fields. A fork normally keeps the original's name.
...
## 11. Test strategy
- **Unit:** the fork determination against a stubbed integration — right parent, wrong parent, not a fork, missing repository, thrown error — asserting that a missing repository is a completed "not a fork", while only a thrown error yields "no decision".
- **DB-integration (`BaseUnitWithDbLayerTest`, method-level `@Sql`):** the four scenarios end to end; that a same-project sibling dependant does *not* block a ban; that banning one of a republisher's artifacts leaves their other artifacts unbanned and still indexed (FR-002); that a second run neither duplicates a ban nor re-processes one; that a candidate whose conflict no longer holds is skipped, while two qualifying members of one conflict are both banned in a single run (FR-006); and that a non-qualifying candidate's reviewer-set `status` and `notes` survive (FR-004).
...
## 13. References
- KTL-4790 — candidate collection, which produces this feature's input.
