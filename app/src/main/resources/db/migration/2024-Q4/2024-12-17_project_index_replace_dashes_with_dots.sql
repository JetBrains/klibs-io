...
WITH package_info AS (
    SELECT project.id,
           ...
           array_to_tsvector(array_agg(DISTINCT replace(pckg.group_id, '-', '.'))) AS group_ids_vector,
           array_to_tsvector(array_agg(DISTINCT replace(pckg.artifact_id, '-', '.'))) AS artifact_ids_vector
    FROM project
...
SELECT project.id AS project_id,
       ...
       (setweight(to_tsvector(owner.login), 'A') ||
        ...
        setweight(to_tsvector(coalesce(array_to_string(pt.tags_processed, ' '), '')), 'B')) AS fts
FROM project
         ...
         LEFT JOIN project_tags pt ON repo.name = pt.project_name;